...
class NightAutomationTestRunner:
    ...
    def test_workflow_files(self):
        """🔄 ワークフローファイル存在確認"""
        required_workflows = ["claude-perfect-automation.yml", "claude-full-automation.yml", "claude-auto-merge.yml"]
        ...
        if not missing_files:
            self.log_result("ワークフローファイル", "PASS", f"{len(required_workflows)}個のワークフローファイル確認")
        else:
            self.log_result("ワークフローファイル", "FAIL", f"不足ファイル: {', '.join(missing_files)}")
    ...
    def test_pr_creation_logic(self):
        ...
        # PR作成ロジックの確認
        pr_patterns = [r"pulls\.create", r"head.*branch", r"base.*main"]
        ...
        if not missing_patterns:
            ...
            self.log_result("PR作成ロジック", "FAIL", f"不足パターン: {', '.join(missing_patterns)}")
    ...
    def test_auto_merge_logic(self):
        ...
        # マージロジックの確認
        merge_patterns = [r"pulls\.merge", r"merge_method", r"squash"]
        ...
        if not missing_patterns:
            ...
            self.log_result("自動マージロジック", "FAIL", f"不足パターン: {', '.join(missing_patterns)}")

    def test_issue_close_logic(self):
        """🔄 Issue自動クローズロジック"""
        workflows_to_check = ["claude-full-automation.yml", "claude-auto-merge.yml"]
