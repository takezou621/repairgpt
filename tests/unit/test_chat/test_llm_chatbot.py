...
try:
    ...
    class RepairContext:
        def __init__(self, device_type=None, device_model=None, issue_description=None, user_skill_level=None):
...
class TestChatbotIntegration:
    ...
    def test_context_switching(self, chatbot):
        """Test switching context mid-conversation"""
        # Initial context
        chatbot.update_context(
            device_type="Xbox Series X",
            issue_description="Overheating"
        )
        
        response1 = chatbot.chat("How do I fix this?")
        assert isinstance(response1, str)
        ...
        response2 = chatbot.chat("What about this new issue?")
        assert isinstance(response2, str)
        
        # Verify context was updated
        assert chatbot.context.device_type == "Nintendo Switch"
        assert chatbot.context.issue_description == "Joy-Con drift"
    
    def test_skill_level_adaptation(self, chatbot):
        ...
        beginner_response = chatbot.chat("How do I fix this?")
        assert isinstance(beginner_response, str)
        
        # Test expert level
        chatbot.update_context(user_skill_level="expert")
        expert_response = chatbot.chat("How do I fix this?")
...
class TestErrorHandling:
    ...
    def test_invalid_device_type(self, chatbot):
        """Test handling of invalid device types"""
        chatbot.update_context(device_type="InvalidDevice")
        response = chatbot.chat("How do I fix this?")
        assert isinstance(response, str)
    
    def test_invalid_skill_level(self, chatbot):
        """Test handling of invalid skill levels"""
        chatbot.update_context(user_skill_level="invalid_level")
        response = chatbot.chat("How do I fix this?")
        assert isinstance(response, str)
    
    def test_very_long_message(self, chatbot):
        """Test handling of very long messages"""
        long_message = "This is a very long message. " * 100
        response = chatbot.chat(long_message)
        assert isinstance(response, str)
    
    def test_special_characters_in_message(self, chatbot):
        """Test handling of special characters in messages"""
        special_message = "My device has 特殊文字 and émojis 🔧🛠️ and symbols @#$%"
        response = chatbot.chat(special_message)
        assert isinstance(response, str)