...
class TestJapaneseSearchEndToEndIntegration:
    ...
    def setup_method(self):
        ...
        with patch('src.services.repair_guide_service.IFixitClient', return_value=self.mock_ifixit_client), \
             ...
             patch('src.services.repair_guide_service.RateLimiter', return_value=self.mock_rate_limiter), \
             patch('src.services.repair_guide_service.OfflineRepairDatabase', return_value=self.mock_offline_db):
...
class TestJapaneseSearchEdgeCasesAndErrorHandling:
    ...
    async def test_japanese_mapper_initialization_failure(self):
        ...
        with patch('src.services.repair_guide_service.get_mapper', side_effect=Exception("Mapper init failed")):
            ...
            with patch.object(service, '_search_ifixit_guides', return_value=[]), \
                 patch.object(service.rate_limiter, 'can_make_request', return_value=True), \
                 patch.object(service.cache_manager, 'get', return_value=None), \
                 patch.object(service, '_enhance_with_related_guides', new_callable=AsyncMock):
    ...
    async def test_japanese_preprocessing_errors(self):
        ...
        with patch.object(self.service.japanese_mapper, 'map_device_name', side_effect=Exception("Mapping failed")), \
    ...
    async def test_network_timeout_with_japanese_queries(self):
        ...
        with patch.object(self.service, '_search_ifixit_guides', side_effect=TimeoutError("Request timeout")), \
             patch.object(self.service.rate_limiter, 'can_make_request', return_value=True), \
             patch.object(self.service.cache_manager, 'get', return_value=None), \
             patch.object(self.service, '_search_offline_guides', new_callable=AsyncMock, return_value=[]), \
             patch.object(self.service, '_enhance_with_related_guides', new_callable=AsyncMock):
    ...
    def test_japanese_filter_edge_cases(self):
        ...
        for difficulty in edge_cases:
            ...
            assert isinstance(result, str) or difficulty is None
        ...
        for category in category_edge_cases:
            ...
            assert result is not None or category is None
            assert isinstance(result, str) or category is None
            ...
            try:
                with patch.object(self.service, '_search_ifixit_guides', return_value=[]), \
                    ...
                    return await asyncio.wait_for(
                        self.service.search_guides(query), 
                        timeout=2.0
                    )
...
class TestJapaneseSearchPerformanceAndLoad:
    ...
    async def test_concurrent_japanese_search_performance(self):
        ...
        with patch.object(self.service, '_search_ifixit_guides', return_value=mock_guides), \
             patch.object(self.service.rate_limiter, 'can_make_request', return_value=True), \
             patch.object(self.service.cache_manager, 'get', return_value=None), \
             patch.object(self.service, '_enhance_with_related_guides', new_callable=AsyncMock):
    ...
    def test_large_batch_japanese_preprocessing(self):
        ...
        base_queries = [
            "スイッチ修理", "アイフォン画面", "プレステコントローラー",
            "ノートパソコン", "スマホバッテリー", "タブレット充電",
        ]
...
class TestJapaneseSearchDataQualityAndConsistency:
    ...
    def test_japanese_device_mapping_consistency(self):
        ...
        for japanese_input, expected_output in consistency_tests:
            ...
            assert mapped == expected_output, f"Inconsistent mapping: '{japanese_input}' -> '{mapped}', expected '{expected_output}'"
    ...
    def test_japanese_filter_normalization_consistency(self):
        ...
        difficulty_tests = [
            ...
            ("かんたん", "easy"),  # Hiragana
            ("初心者", "beginner"),
            ("しょしんしゃ", "beginner"),  # Hiragana
            ("普通", "moderate"),
            ("ふつう", "moderate"),  # Hiragana
        ]
        
        for japanese_input, expected_output in difficulty_tests:
            normalized = filters.normalize_japanese_difficulty(japanese_input)
            assert normalized == expected_output, f"Inconsistent difficulty normalization: '{japanese_input}' -> '{normalized}'"
        
        # Test category consistency
        category_tests = [
            ("画面修理", "screen repair"),
            ("がめんしゅうり", "screen repair"),  # Hiragana
            ("バッテリー交換", "battery replacement"),
            ("ばってりーこうかん", "battery replacement"),  # Hiragana
        ]
        
        for japanese_input, expected_output in category_tests:
            normalized = filters.normalize_japanese_category(japanese_input)
            assert normalized == expected_output, f"Inconsistent category normalization: '{japanese_input}' -> '{normalized}'"
    ...
    def test_data_integrity_after_japanese_processing(self):
        ...
        filters = SearchFilters(
            difficulty_level=original_data["filters"]["difficulty"],
            category=original_data["filters"]["category"]
        )
...
class TestJapaneseSearchBackwardCompatibility:
    ...
    async def test_english_search_functionality_preserved(self):
        ...
        with patch.object(self.service, '_search_ifixit_guides', return_value=english_guides), \
             patch.object(self.service.rate_limiter, 'can_make_request', return_value=True), \
             patch.object(self.service.cache_manager, 'get', return_value=None), \
             patch.object(self.service, '_enhance_with_related_guides', new_callable=AsyncMock):
    ...
    def test_cache_key_generation_compatibility(self):
        ...
        english_filters = SearchFilters(device_type="iPhone", difficulty_level="easy")
        japanese_filters = SearchFilters(device_type="アイフォン", difficulty_level="簡単")
    ...
    async def test_service_initialization_backward_compatibility(self):
        ...
        with patch('src.services.repair_guide_service.IFixitClient'), \
            ...
            service_without_japanese = RepairGuideService(
                ifixit_api_key="test_key",
                enable_japanese_support=False
            )
            ...
            with patch.object(service_without_japanese, '_search_ifixit_guides', return_value=[]), \
                 patch.object(service_without_japanese.rate_limiter, 'can_make_request', return_value=True), \
                 patch.object(service_without_japanese.cache_manager, 'get', return_value=None), \
                 patch.object(service_without_japanese, '_enhance_with_related_guides', new_callable=AsyncMock):
    ...
    def test_confidence_scoring_backward_compatibility(self):
        ...
        for query in test_queries:
            score = self.service._calculate_confidence_score(mock_guide, query, SearchFilters())
...
class TestJapaneseSearchRealWorldScenarios:
    ...
    async def test_beginner_japanese_user_scenario(self):
        ...
        filters = SearchFilters(
            difficulty_level="初心者",  # "beginner"
            device_type="スイッチ"      # "Switch"
        )
        
        with patch.object(self.service, '_search_ifixit_guides', side_effect=mock_search), \
             patch.object(self.service.rate_limiter, 'can_make_request', return_value=True), \
             patch.object(self.service.cache_manager, 'get', return_value=None), \
             patch.object(self.service, '_enhance_with_related_guides', new_callable=AsyncMock):
    ...
    async def test_specific_problem_japanese_user_scenario(self):
        ...
        filters = SearchFilters(category="コントローラー修理")  # "controller repair"
    ...
    async def test_mixed_language_tech_savvy_user_scenario(self):
        ...
        filters = SearchFilters(
            device_type="アイフォン15",  # "iPhone 15"
            category="battery replacement"  # Mixed English category
        )
        ...
        iphone_battery_results = [
            r for r in results 
            if "iPhone 15" in r.guide.device and "Battery" in r.guide.title
        ]
    ...
    async def test_emergency_repair_scenario(self):
        ...
        filters = SearchFilters(
            difficulty_level="簡単",  # "easy" - want quick fix
            max_time="30分"  # "30 minutes"
        )
    ...
    async def test_professional_repair_shop_scenario(self):
        ...
        filters = SearchFilters(
            difficulty_level="上級者",  # "expert"
            include_community_guides=True,
            min_rating=4.5
        )
    ...
    def test_japanese_error_message_scenario(self):
        ...
        filters = SearchFilters(difficulty_level="未知の難易度")  # "unknown difficulty"
