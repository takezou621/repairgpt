...
from data.offline_repair_database import offline_repair_db
...
class IFixitIntegration:
    ...
    def _cache_key(self, query: str, device_type: str = "", difficulty: str = "") -> str:
    ...
    async def search_guides(
        ...
        try:
            ...
            if self.api_key:
                try:
                    guides = await self._search_api_guides(query, device_type, difficulty)
    ...
    def _search_offline_guides(
        ...
        offline_guides = offline_repair_db.search_guides(
            query=query,
    ...
    async def get_guide_details(self, guide_id: int) -> Dict[str, Any]:
        ...
        try:
            guide_details = await asyncio.to_thread(
                self.client.get_guide, 
                guide_id
            )
...
async def demo_ifixit_integration():
    ...
    if results["success"]:
        ...
        for i, guide in enumerate(results["guides"][:2], 1):
            ...
            print(f"      Tools: {', '.join(guide['tools'][:3])}")
    else:
    ...
    if results2["success"]:
        print(f"✅ Retrieved from {results2['source']} (should be cache if first succeeded)")
