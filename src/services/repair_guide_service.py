...
class RepairGuideService:
            ...
            try:
                ...
                for guide in offline_guides:
                    result = RepairGuideResult(
                        guide=guide,
                        source='offline',
                        confidence_score=self._calculate_confidence_score(guide, query, filters) * 0.8,  # Lower confidence for offline
    ...
    async def get_trending_guides(self, limit: int = 10) -> List[RepairGuideResult]:
        ...
        if len(results) < limit:
            popular_queries = ["iPhone screen", "Nintendo Switch", "laptop battery", "headphones"]
    ...
    def _calculate_confidence_score(self, guide: Guide, query: str, filters: SearchFilters) -> float:
        ...
        popular_devices = ['iphone', 'android', 'switch', 'macbook', 'xbox', 'playstation']
        ...
        for result in results:
            try:
                ...
                if result.guide.device:
                    ...
                    related = [r.guide for r in related_guides 
                             if r.guide.guideid != result.guide.guideid][:2]
