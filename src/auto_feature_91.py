...
from database.models import (
    User, Device, Issue, RepairGuide, ChatSession, RepairAttempt
)
from database.crud import (
    UserCRUD, DeviceCRUD, RepairGuideCRUD, ChatSessionCRUD,
    RepairAttemptCRUD, StatisticsCRUD
)
...
class SQLiteDatabaseIntegration:
        ...
        try:
            with get_db_session() as db:
                attempt = RepairAttemptCRUD.create_repair_attempt(
                    ...
                    issue_id=issue_id,
                    user_id=uuid.UUID(user_id) if user_id else None,
                    repair_guide_id=uuid.UUID(repair_guide_id) if repair_guide_id else None
                )
    ...
    def get_user_repair_history(self, user_id: str, limit: int = 10) -> List[Dict[str, Any]]:
        """
            ...
            with get_db_session() as db:
                ...
                for attempt in attempts:
                    history.append({
                        ...
                        "completion_rate": float(attempt.completion_rate) if attempt.completion_rate else 0.0,
                        "rating": attempt.rating,
                        "started_at": attempt.started_at.isoformat() if attempt.started_at else None,
                        "completed_at": attempt.completed_at.isoformat() if attempt.completed_at else None
                    })
                
                logger.info(f"Retrieved {len(history)} repair history records for user {user_id}")
                return history
            ...
            with get_db_session() as db:
                ...
                for guide in guides:
                    results.append({
                        ...
                        "success_rate": float(guide.success_rate) if guide.success_rate else 0.0,
