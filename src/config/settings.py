...
class Settings(BaseSettings):
    ...
    ifixit_api_key: Optional[str] = Field(
        default=None,
        description="iFixit API key for repair guides"
    )
    ...
    ifixit_api_base: str = Field(
        default="https://www.ifixit.com/api/2.0",
        description="iFixit API base URL"
    )
    ...
    for service, key in api_keys.items():
        if key:
            if service == "openai" and key.startswith("sk-"):
                ...
                results[service] = {"valid": False, "configured": True, "error": "Invalid format"}
        else:
            results[service] = {"valid": False, "configured": False, "error": "Not configured"}
