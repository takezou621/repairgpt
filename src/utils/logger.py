...
class LoggerMixin:
    ...
    def log_error(self, error: Exception, context: str = "", **kwargs):
        ...
        self.logger.error(f"Error in {context}: {str(error)}", 
                         exc_info=True, extra={"extra_data": extra_data})
...
def setup_logging(log_dir: str = "logs", 
                 log_level: str = None,
                 json_format: bool = None) -> RepairGPTLogger:
    """
    ...
    init_logger.info(f"RepairGPT logging system initialized - Level: {log_level}, Format: {'JSON' if json_format else 'TEXT'}")
...
def log_api_error(logger: logging.Logger, endpoint: str, error: Exception, **kwargs):
    ...
    logger.error(f"API error: {endpoint} - {str(error)}", 
                exc_info=True, extra={"extra_data": kwargs})
