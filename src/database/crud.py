...
class RepairGuideCRUD:
    ...
    def get_repair_guide_by_id(db: Session, guide_id: Union[str, uuid.UUID]) -> Optional[RepairGuide]:
        ...
        return (
            ...
            .options(selectinload(RepairGuide.repair_steps))
            .filter(RepairGuide.id == guide_id)
            .first()
        )
    ...
    def search_repair_guides(
        db: Session, query: str, device_id: Optional[str] = None, limit: int = 10
    ) -> List[RepairGuide]:
...
class RepairAttemptCRUD:
    ...
    def get_user_repair_history(db: Session, user_id: Union[str, uuid.UUID], limit: int = 10) -> List[RepairAttempt]:
        ...
        return (
            ...
            .filter(RepairAttempt.user_id == user_id)
            .order_by(RepairAttempt.started_at.desc())
            .limit(limit)
            .all()
        )
...
class StatisticsCRUD:
    ...
    def get_repair_success_rate_by_device(db: Session) -> List[Dict[str, Any]]:
        ...
        results = (
            ...
            .join(Device)
            .filter(RepairAttempt.status == "completed")
            .group_by(RepairAttempt.device_id, Device.name)
            .all()
        )
    ...
    def get_popular_devices(db: Session, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        results = (
            ...
            .join(Device)
            .filter(ChatSession.device_id.isnot(None))
            .group_by(ChatSession.device_id, Device.name)
